...
| `EfficientATMel.compute` | [efficient-at-mn10-coreml](https://huggingface.co/schism-audio/efficient-at-mn10-coreml) (+ mn40) | `test_vectors_logmel.npz` |
| `BeatThisMel.compute` / `.melBanks` | [beat-this-coreml](https://huggingface.co/schism-audio/beat-this-coreml) | `test_vectors_beatmel.npz` |
| `BasicPitchCQT.magnitude/.features` + `.harmonicStack` | [basic-pitch-coreml](https://huggingface.co/schism-audio/basic-pitch-coreml) | `test_vectors_cqt.npz` |
...
The three log-mel frontends are three different filterbank conventions —
`LogMel` (librosa Slaney scale *with* Slaney area norm, power spectrum),
`EfficientATMel` (Kaldi banks, zeroed Nyquist column, symmetric win-800 Hann
centered into n_fft 1024), `BeatThisMel` (torchaudio Slaney scale with
`norm=None`, *magnitude* STFT scaled `1/sqrt(n_fft)`) — none interchangeable.
`BasicPitchCQT` is a full CQT2010v2 multirate chain (9 octaves of complex
kernels + decimate-by-2) with per-window `NormalizedLog` and harmonic
stacking; every fixed constant is recomputed in `init` rather than shipped
as a blob. `STFT.forward/inverse` also accept an explicit `window:` —
SCNet calls `torch.stft` with *no* window, i.e. pass all-ones (SCNet's
vectors are not published; the rectangular path is covered by a pure-Swift
round-trip test).
...
floor. Log-domain and min/max-normalized outputs are gated by an absolute
tolerance instead (their references cross zero, so rtol cannot carry the
comparison); each test comments the observed worst-case diff its atol was
derived from. The pipeline vectors use a deterministic mock model (a 2-tap FIR with
